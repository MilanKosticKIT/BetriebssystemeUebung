...
\begin{document}
\begin{titlepage}
\title{Betriebsysteme Übung: \\
	Dateisystem unter Linux mithilfe von FUSE}
\author{Milan Kostic, komi1029 \and Joscha Völler, vojo1016 \and Hannes Illy, ilha1011 \and Alexander Grining, gral1025}
...
Beim Testen fanden wir vor, dass unsere die Dateien in unserem Dateisystem teilweise zerstört werden. Unser Testvideo konnte man abspielen, jedoch wies das Video viele Pixelfehler auf. Der Fehler trat sowohl beim Lesen als auch beim Schreiben auf.
Im Debuggen fanden wir heraus, dass in regelmäßigen Abständen ein unleserlicher Block gelesen / geschrieben wurde.  
Als Ursache des Bugs fanden wir einen unberücksichtigten Fall in der read / write Methode, genauer gesagt beim Berechnen der Größe des letzten zu lesenden / schreibenden Blocks, dabei benutzten wir die Modulo Funktion mit unserer Block Größe. Dadurch wurde, falls der Block genau so groß war wie die vordefinierten Block Größe, nicht gelesen / geschrieben. Behoben wurde der Fehler durch hinzufügen einer extra IF-Abfrage genau zu diesem Fall, also falls das Ergebnis der Modulo Funktion gleich null ist, wird genau die Block Größe eingelesen, da wir ja durch vorige Berechnung wissen, dass da noch zu lesende / schreibende Daten sind.    
